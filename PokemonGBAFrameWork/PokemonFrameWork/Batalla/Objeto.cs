...
namespace PokemonGBAFrameWork
{
    public class Objeto
    {
        ...
        public static Objeto GetObjeto(RomGba rom, int index)
        {
            BloqueImagen blImg;
        ...
        {
            Objeto[] objetos = new Objeto[GetTotal(rom)];
            for (int i = 0; i < objetos.Length; i++)
                objetos[i] = GetObjeto(rom, i);
            return objetos;
        }

        public static void SetObjeto(RomGba rom, int index, Objeto objeto)
        {
            int offsetImagenYPaleta;
            int offsetDatos = Zona.GetOffsetRom(ZonaDatosObjeto, rom).Offset + index * (int)LongitudCampos.Total;
            ...
            Word.SetData(rom, offsetDatos, objeto.Index);//por mirar
            ...
            offsetDatos += OffsetRom.LENGTH;
            DWord.SetData(rom, offsetDatos, objeto.BattleUsage);
            offsetDatos += DWord.LENGTH;
            if (objeto.PointerBattleUsage != null)
                OffsetRom.SetOffset(rom.Data.Bytes, offsetDatos, objeto.PointerFieldUsage);
            offsetDatos += OffsetRom.LENGTH;
            DWord.SetData(rom, offsetDatos, objeto.ExtraParameter);
